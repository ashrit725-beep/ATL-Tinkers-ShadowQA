...
let emergentOverlay;
// The project's .env is authoritative for the overlay switch (ShadowQA owns runtime-error UX in this app).
const projectEnv = (() => {
  try {
    return require("dotenv").parse(require("fs").readFileSync(path.join(__dirname, ".env")));
  } catch {
    return {};
  }
})();
const overlayDisabled = (projectEnv.DISABLE_EMERGENT_OVERLAY ?? process.env.DISABLE_EMERGENT_OVERLAY) === "true";
if (isDevServer && !overlayDisabled) {
  try {
...
webpackConfig.devServer = (devServerConfig) => {
  // ShadowQA owns runtime-error UX; keep webpack's overlay for compile errors only.
